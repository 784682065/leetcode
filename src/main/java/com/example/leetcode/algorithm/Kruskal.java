 ...
 * @Author: huzp
 * @Description:
 * @Date: Created in  10:04 2020/5/2
 * @param:
 */
/**
...
public class Kruskal {
    ...
    public static void kruskal(int[][] arcs) {
        ...
        while(!finished) {
            ...
            //找到了最小值，设置连通标记 , 并查集
            if(group[n1] == 0 && group[n2] == 0){ //两个都没加入过集合.合并为一个集合
                group[n1] = groupNum;
                group[n2] = groupNum;
                groupNum++;
            }
            else if(group[n1] > 0 && group[n2] > 0) { //如果两个大集合,合并为一个集合
                int tmp = group[n2];
            ...
            for(int i = 0; i < group.length; i++) {
                if(group[i] != group[0]){  //只有一个集合时候才break掉
                    finished = false;
                    break;
                }
                else{
